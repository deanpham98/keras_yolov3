...
from yolo3.model import (preprocess_true_boxes, yolo_body,yolo_eval, yolo_head, yolo_loss)
...
def _main(args):
    ...
    classes_path =arg.classes_path
    anchors_path=arg.anchors_path

    class_names = arg.get_classes(classes_path)
    anchors = arg.get_anchors(anchors_path)
...
def train(model, class_names, anchors, image_data, boxes, detectors_mask, matching_true_boxes, validation_split=0.1):
    ...
    model.compile(optimizer='adam',
                  loss={'yolo_loss': lambda y_true, y_pred: y_pred})  # This is a hack to use the custom loss function in the last layer.
